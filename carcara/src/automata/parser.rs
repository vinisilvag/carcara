...
mod tests {
    ...
    fn test_parses_range_transition() {
        ...
        match transition.trigger {
            ...
            _ => panic!("expected Trigger::Range"),
        }
    }

    // TODO: checar como sao transicoes epsilon no singleton da string
    // #[test]
    // fn test_parses_epsilon_transition() {
    //     let input = r#"automaton a { init s0; s0 -> s1 eps; accepting s1; };"#;
    //
    //     let (_, automaton) = parse_automaton(input).unwrap();
    //     let s0 = &automaton.all_states[0];
    //
    //     let transition = s0.transitions.iter().next().unwrap();
    //     assert!(matches!(transition.trigger, Trigger::Epsilon));
    // }
