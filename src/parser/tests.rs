...
pub fn parse_terms<const N: usize>(
    ...
    let mut parser = Parser::new(pool, TEST_CONFIG, definitions.into()).expect(ERROR_MESSAGE);
    parser.parse_problem().expect(ERROR_MESSAGE);

    terms.map(|s| {
        parser.reset(s.into()).expect(ERROR_MESSAGE);
...
fn test_hash_consing() {
    ...
    let mut parser = Parser::new(&mut pool, Config::new(), input.into()).unwrap();
...
fn test_define_fun_rec() {
    fn run_test(pool: &mut PrimitivePool, problem: &str, expected_premises: &[&str]) {
        ...
        for &p in expected_premises {
            parser.reset(p.into()).expect(ERROR_MESSAGE);
